...
# Load
load('Results/Summary/EXP4/g8/pixel_xclass_summary_bysp_w_runs_1_50.Rdata')
load('Results/Summary/EXP4/g8/pixel_xclass_summary.Rdata')


# pixelSummary("Results/EXP4/g8/d-g8_hp-0.5", "d-g8_hp-0.5", run=1:20, "Results/Plots/EXP4", plot.pdf = TRUE)
...
ggsave("Results/Plots/EXP4/g8/FigS1rawcore.pdf", height = 14, width = 16)
...
fig4 <- plot_grid(tcore + theme(legend.position="right"),
                  ...
                  label_size = 30,
                  nrow = 2) 
# ggsave("Results/Plots/EXP4/g8/Fig4_t_core_trans.pdf", height = 17, width = 14)
...
tcore_det_single = xclass.sp %>%
  ...
  theme(axis.title.x=element_text(size=30),axis.title.y=element_text(size=30), xis.text.x=element_text(size=25),axis.text.y=element_text(size=25), legend.text=element_text(size=25, hjust = 1, vjust = 0.5), legend.title=element_text(size = 25)) 
...
grid <- plot_grid(tcore_det_single + theme(legend.position="right"),
                  ...
                  # hjust = -2,
                  nrow = 2) 
# ggsave("Results/Plots/EXP4/g8/Fig4_lines.pdf", height = 14, width = 18)  
...
plot_grid(core, 
          ...
          align = 'hv',
          nrow = 2)
ggsave("Results/Plots/EXP4/g8/Fig4_grid_lines.pdf", height = 14, width = 30)  
...
grid <- plot_grid(ncore + theme(legend.position="right"),
                  ...
                  label_size = 30,
                  nrow = 2) 
# ggsave("Results/Plots/EXP4/g8/Fig5_n_core_trans.pdf", height = 18, width = 14)  
...
grid <- plot_grid(tcore_det_single + theme(legend.position="right"),
                  ...
                  # hjust = -2,
                  nrow = 2) 
ggsave("Results/Plots/EXP4/g8/Fig5_lines.pdf", height = 14, width = 18)  
...
plot_grid(core, 
          ...
          align = 'hv',
          nrow = 2)
ggsave("Results/Plots/EXP4/g8/Fig5_grid_lines.pdf", height = 14, width = 30)  
...
xclass.ngrid = c()
for(r in 1:50){
  tmp = pixelXclassBySpecies("Results/EXP4/d-g8_hp-0.5", "d-g8_hp-0.5", run=r, scale = 3, t_window = 186:200, ct_threshold = 1/3)
  xclass.ngrid = rbind(xclass.ngrid, tmp)
}
xclass.ngrid.filter <- filter(xclass.ngrid, pix == 500 & p == 0.5)
# only saving the filtered version
# write.csv(xclass.ngrid.filter, "Results/Summary/EXP4/d-g8_hp-0.5/pixel_xclass_summary_bysp.Rdata")


load('Results/Summary/EXP4/d-g8_hp-0.5/pixel_xclass_summary_bysp.Rdata')
...
vplot <- xclass.ngrid.filter %>%
  ...
  mutate(correct = case_when(xc == "c-c" ~ 1,
                             xc == "c-t" ~ 0))
# write.csv(vplot, "Results/EXP4/d-g8_hp-0.5/vplot.csv", row.names = FALSE)
...
ggsave("Results/Plots/EXP4/g8/Fig6.pdf", height = 10, width = 12)
